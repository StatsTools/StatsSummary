...
#' calc_gmt(data = data, var_expr = titer, group_expr = group, decimals = 2, groups_order = c('2', '1'), method_ci = 't.test', ci_sep = ";", dec_sep = ".", base = exp(1))
calc_gmt <- function(data, var_expr, group_expr, method_ci = "t.test", decimals = 1, groups_order = NA_character_, nboot = 1000, ci_sep = '\u2012', dec_sep = ".", base = exp(1)) {
# Validation Step -------------------------------------------------------------
...
# Development Step ------------------------------------------------------------
  ...
  result <- data |>
    ...
    dplyr::rowwise() |>
    dplyr::mutate(
      N = roundmath_str(`Nº Participantes`, 0),
      RES = ifelse(is.na(Média), 'NC', roundmath_str(Média, decimals)),
      CI95 = ifelse(is.na(`Limite Inferior`) | is.na(`Limite Superior`), 'NC', paste0('(', roundmath_str(`Limite Inferior`, decimals), ' ', ci_sep,' ', roundmath_str(`Limite Superior`, decimals), ')'))
    ) |>
    dplyr::ungroup() |>
